...
(async() => {
  ...
  const sorted = _.sortBy(
    list,
    listing => listing.p3_event_data_logging.cleanliness_rating
  );
  for (var i = 0; i < sorted.length; i++) {
    const listing = sorted[i];
    const rating = listing.p3_event_data_logging.cleanliness_rating;
    const file = getUri(`listing-photos/${listing.id}/0.jpg`);
    const newFile = getUri(`by-cleanliness/${rating}/${listing.id}.jpg`);
